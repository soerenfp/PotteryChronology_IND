...
rim_counts_collapsed <- ceramics_data %>%
  ...
  mutate(rim_group = case_when(
    ...
    rim_category == "Inverted" ~ "Inverted rims",
    TRUE ~ "Other"
  )) %>%
  group_by(rim_group) %>%
...
OBS: I need to work on separating the Inwards thickened category from the Inverted category
...
These are flasks and I only have one registration!
...
Interpretation: I must remember that many of these vessels are not fully reconstructed! It is the base that is missing from many of these, so the height is not valid. I must make new calculations only for the fully reconstructed vessels!

Make a new calculation only based on the R-values in the excel sheet!
