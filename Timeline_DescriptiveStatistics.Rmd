...
size_counts <- data.frame(table(ceramics_data$size_category))
...
legend_table_size <- data.frame(
  ...
  Description = c("0-1 l", "1-6 l", "6-35 l", "35-100 l", ">100 l")
)
...
# Count occurrences of type_category per site AND date
data_count_VS01 <- ceramics_data %>%
  group_by(site_name, date, type_category) %>%
  summarise(count = n(), .groups = 'drop')
...
###############################################################
# 🔹 Plot function: one plot per site AND date
###############################################################

plot_site_date_VS01 <- function(site_name_input, date_input) {
  
  site_data <- data_count_VS01 %>%
    filter(site_name == site_name_input, date == date_input)
  
  ggplot(site_data, aes(x = type_category, y = count)) +
  geom_bar(
    stat = "identity",
    position = "dodge",
    fill = "lightblue",
  ...
  theme_minimal() +
  labs(
    title = paste("Vessel Types at Site:", site_name_input, "| Date:", date_input),
    x = "Type Category",
    y = "Count"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
###############################################################
# 🔹 Generate plots for ALL site × date combinations
###############################################################

site_date_combinations <- data_count_VS01 %>%
  distinct(site_name, date)

for (i in 1:nrow(site_date_combinations)) {
  print(
    plot_site_date_VS01(
      site_date_combinations$site_name[i],
      site_date_combinations$date[i]
    )
  )
}
```
...
# =========================================================
# STEP 1: COUNT DATA (STRICT HIERARCHY)
# =========================================================

data_count_SS02 <- ceramics_data %>%
  group_by(site_name, date, size_category) %>%
  summarise(count = n(), .groups = 'drop')

# =========================================================
# STEP 2: COMPLETE WITHIN SITE × DATE (NOT GLOBAL)
# =========================================================

data_count_SS02 <- data_count_SS02 %>%
  group_by(site_name, date) %>%   # 🔥 KEY FIX
  complete(size_category = c("a", "b", "c", "d", "e"), fill = list(count = 0)) %>%
  ungroup()
...
data_count_SS02 <- data_count_SS02 %>%
  mutate(
    size_category = factor(size_category, levels = c("a", "b", "c", "d", "e"))
  )
...
plot_site_date_SS02 <- function(site_name_input, date_input) {
  
  site_data <- data_count_SS02 %>%
    filter(site_name == site_name_input, date == date_input)
  
  ggplot(site_data, aes(x = size_category, y = count)) +
    geom_bar(
      stat = "identity",
      fill = "lightblue",
      color = "black"
    ...
    theme_minimal() +
    labs(
      title = paste("Size Distribution | Site:", site_name_input, "| Phase:", date_input),
      x = "Size Category",
      y = "Count"
    ) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
site_date_combinations_SS02 <- data_count_SS02 %>%
  distinct(site_name, date)

for (i in seq_len(nrow(site_date_combinations_SS02))) {
  print(
    plot_site_date_SS02(
      site_date_combinations_SS02$site_name[i],
      site_date_combinations_SS02$date[i]
    )
  )
}
```
...
# =========================================================
# STEP 1: COUNT DATA (site × date × rim_category)
# =========================================================

data_count_RS02 <- ceramics_data %>%
  group_by(site_name, date, rim_category) %>%
  summarise(count = n(), .groups = 'drop')

# =========================================================
# STEP 2: COMPLETE WITHIN SITE × DATE (CRITICAL)
# =========================================================

rim_levels <- c(
  "Sr", "Sp", "Sf", "Sb", "Es", "Ec", "Ecc", "Ef", "Efl", "Ep",
  "Et", "Esg", "Emg", "Vs", "Vc", "Oth", "Otc", "Otf", "Ith",
  "Itc", "Itf", "Th", "Tc", "Tf", "Ca", "B", "Y"
)
...
data_count_RS02 <- data_count_RS02 %>%
  ...
  complete(rim_category = rim_levels, fill = list(count = 0)) %>%
  ungroup()
...
data_count_RS02 <- data_count_RS02 %>%
  mutate(
    rim_category = factor(rim_category, levels = rim_levels)
  )
...
plot_site_phase_RS02 <- function(site_name_input, date_input) {
  
  site_data <- data_count_RS02 %>%
    filter(site_name == site_name_input, date == date_input)
  
  ggplot(site_data, aes(x = rim_category, y = count)) +
    geom_bar(
      stat = "identity",
      fill = "lightblue",
      color = "black"
    ...
    theme_minimal() +
    labs(
      title = paste("Rim Counts | Site:", site_name_input, "| Phase:", date_input),
      x = "Rim Category",
      y = "Count"
    ) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
site_phase_combinations_RS02 <- data_count_RS02 %>%
  distinct(site_name, date)

for (i in seq_len(nrow(site_phase_combinations_RS02))) {
  print(
    plot_site_phase_RS02(
      site_phase_combinations_RS02$site_name[i],
      site_phase_combinations_RS02$date[i]
    )
  )
}
```
...
-----------------------------
# Site, date and vessel bases
-----------------------------

This analysis only included the actual base count, so no rounded bases were added artificially. 
...
# =========================================================
# STEP 0: CLEAN DATA FIRST (IMPORTANT FIX)
# =========================================================

ceramics_clean <- ceramics_data %>%
  mutate(
    base_category = trimws(base_category)
  ) %>%
  filter(
    !is.na(base_category),
    base_category != "",
...
# =========================================================
# STEP 1: COUNT (site × date × base_category)
# =========================================================

data_count_BS02 <- ceramics_clean %>%
  group_by(site_name, date, base_category) %>%
  summarise(count = n(), .groups = "drop")

# =========================================================
# STEP 2: COMPLETE WITHIN SITE × DATE
# =========================================================

base_levels <- sort(unique(data_count_BS02$base_category))

data_count_BS02 <- data_count_BS02 %>%
  group_by(site_name, date) %>%
  complete(base_category = base_levels, fill = list(count = 0)) %>%
  ungroup()
...
data_count_BS02 <- data_count_BS02 %>%
  mutate(
    base_category = factor(base_category, levels = base_levels)
  )
...
plot_site_phase_BS02 <- function(site_name_input, date_input) {
  
  site_data <- data_count_BS02 %>%
    filter(site_name == site_name_input, date == date_input)
  
  ggplot(site_data, aes(x = base_category, y = count)) +
    geom_bar(
      stat = "identity",
      fill = "lightblue",
      color = "black"
    ...
    theme_minimal() +
    labs(
      title = paste("Base Counts | Site:", site_name_input, "| Phase:", date_input),
      x = "Base Category",
      y = "Count"
    ) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# =========================================================
# STEP 5: LOOP THROUGH ALL SITE × PHASE
# =========================================================

site_phase_combinations_BS02 <- data_count_BS02 %>%
  distinct(site_name, date)

for (i in seq_len(nrow(site_phase_combinations_BS02))) {
  print(
    plot_site_phase_BS02(
      site_phase_combinations_BS02$site_name[i],
      site_phase_combinations_BS02$date[i]
    )
  )
}
```
...
rim_description_map <- data.frame(
  ...
  rim_description = c("Simple, rounded", "Simple, pointy", "Simple, flattened", 
                      ...
                      "Everted, collared channelled", "Everted, flaring", 
                      "Everted, flattened", "Everted, protruding", 
                      "Everted, thickened", "Everted, single grove", 
                      "Everted, multiple groove", "Vertical, simple", "Vertical, collared", 
                      "Outward thickened, hinged", "Outward thickened, curved", 
...
size_description_map <- data.frame(
  ...
  size_description = c("0-1 l", "1-6 l", "6-35 l", "35-50 l", ">50 l")
)
...
-----------------------------
# plot of attributes per site
-----------------------------

In the following I will explore which attribute cateogries are dominant per site

OBS I will use the AS abbreviations for Size - Site and then followed by a data number -> ex-SS01
...
# Count the occurrences of size_category per site, excluding NA values
data_count_AS01 <- ceramics_subset %>%
  filter(!is.na(att_category)) %>%  # Exclude rows with NA in size_category
  group_by(site_name, att_category) %>%
  summarise(count = n(), .groups = 'drop')

# Create a mapping of base_category to descriptions (optional for additional info)
att_description_map <- data.frame(
  att_category = c("H", "Sp", "St", "K", "Pl"),
  att_description = c("Handles", "Spouts", "Steamers", "Knobs", "Plastic band")
)

# Ensure `size_category` columns are character for merging
data_count_AS01$att_category <- as.character(data_count_AS01$att_category)
att_description_map$att_category <- as.character(att_description_map$att_category)

# Merge the descriptions with the dataset
data_count_AS01 <- data_count_AS01 %>%
  left_join(att_description_map, by = "att_category")

# Ensure the x-axis and legend are dynamically ordered based on present base types
data_count_AS01 <- data_count_AS01 %>%
  mutate(
    att_category = factor(att_category, levels = unique(data_count_AS01$att_category)),
    att_description = factor(att_description, levels = unique(data_count_AS01$att_description))
  )

# Create a function to generate a plot for a single site
plot_site_AS01 <- function(site_name) {
  site_data <- data_count_AS01 %>% filter(site_name == !!site_name)
  
  ggplot(site_data, aes(x = att_category, y = count, fill = att_description)) +
    geom_bar(stat = "identity", position = "dodge") +
    scale_fill_viridis(discrete = TRUE, option = "D") +
    theme_minimal() +
    labs(
      title = paste("Attribute Categories at Site:", site_name),
      x = "Attribute Category",
      y = "Count",
      fill = "Attribute Type"
    ) +
...
# Get unique site names
site_names_AS01 <- unique(data_count_AS01$site_name)

# Generate and print plots for each site
for (site in site_names_AS01) {
  print(plot_site_AS01(site))
}
```

OBS: Too few attributes to be interesting


# Count of rim, size, base, and attributes per vessel type
...
# =========================================================
# STEP 1: GENERIC COUNT FUNCTION
# =========================================================

make_counts <- function(data, var_name, label) {
  data %>%
    mutate(
      category = as.character(.data[[var_name]])
    ) %>%
    filter(!is.na(category), category != "", category != "special") %>%
    group_by(type_category, category) %>%
    summarise(count = n(), .groups = "drop") %>%
    mutate(attribute = label)
}
...
rim_df  <- make_counts(ceramics_data, "rim_collapsed", "Rim")
base_df <- make_counts(ceramics_data, "base_category", "Base")
att_df  <- make_counts(ceramics_data, "att_category", "Attribute")
size_df <- make_counts(ceramics_data, "size_category", "Size")

# =========================================================
# STEP 3: COMBINE EVERYTHING
# =========================================================

combined_summary <- bind_rows(rim_df, base_df, att_df, size_df)

# =========================================================
# STEP 4: COMPLETE (ensures consistent structure per type)
# =========================================================

combined_summary <- combined_summary %>%
  group_by(type_category, attribute) %>%
  complete(category, fill = list(count = 0)) %>%
  ungroup()
...
# =========================================================
# STEP 1: BUILD LONG FORMAT DATA
# =========================================================

rim_df <- ceramics_data %>%
  filter(!is.na(rim_collapsed), rim_collapsed != "", rim_collapsed != "special") %>%
  group_by(type_category, category = rim_collapsed) %>%
  summarise(count = n(), .groups = "drop") %>%
  mutate(attribute = "Rim")

base_df <- ceramics_data %>%
  filter(!is.na(base_category), base_category != "", base_category != "special") %>%
  group_by(type_category, category = base_category) %>%
  summarise(count = n(), .groups = "drop") %>%
  mutate(attribute = "Base")

att_df <- ceramics_data %>%
  filter(!is.na(att_category), att_category != "", att_category != "special") %>%
  group_by(type_category, category = att_category) %>%
  summarise(count = n(), .groups = "drop") %>%
  mutate(attribute = "Attribute")

size_df <- ceramics_data %>%
  filter(!is.na(size_category)) %>%
  group_by(type_category, category = size_category) %>%
  summarise(count = n(), .groups = "drop") %>%
  mutate(attribute = "Size")

all_data <- bind_rows(rim_df, base_df, att_df, size_df)

# =========================================================
# STEP 2: PLOT FUNCTION (ONE TYPE = ONE FIGURE)
# =========================================================

plot_type_profile <- function(type_id) {
  
  df <- all_data %>%
    filter(type_category == type_id)
  ...
  ggplot(df, aes(x = category, y = count)) +
    ...
    theme_minimal() +
    labs(
      title = paste("Vessel Type", type_id, "— Morphological Profile"),
      x = NULL,
      y = "Count"
    ) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
type_list <- sort(unique(ceramics_data$type_category))

for (t in type_list) {
  print(plot_type_profile(t))
}
```

# rim, base and attribute dependent on type and size
...
clean_data <- ceramics_data %>%
  mutate(
    type_category = as.character(type_category),
    size_category = as.character(size_category),

    rim_collapsed = na_if(as.character(rim_collapsed), ""),
    base_category = na_if(as.character(base_category), ""),
    att_category  = na_if(as.character(att_category), "")
  ) %>%
  filter(!is.na(type_category), !is.na(size_category))

# =========================================================
# STEP 2: PLOT FUNCTION (STRICT SEPARATION)
# =========================================================

plot_type_size <- function(type_input, size_input) {
  
  df <- clean_data %>%
    filter(type_category == type_input,
           size_category == size_input)
  
  # --- RIM (NOW rim_collapsed) ---
  rim_df <- df %>%
    filter(!is.na(rim_collapsed)) %>%
    group_by(category = rim_collapsed) %>%
    summarise(count = n(), .groups = "drop") %>%
    mutate(variable = "Rim (collapsed)")
  
  # --- BASE ---
  base_df <- df %>%
    filter(!is.na(base_category)) %>%
    group_by(category = base_category) %>%
    summarise(count = n(), .groups = "drop") %>%
    mutate(variable = "Base")
  
  # --- ATTRIBUTE ---
  att_df <- df %>%
    filter(!is.na(att_category)) %>%
    group_by(category = att_category) %>%
    summarise(count = n(), .groups = "drop") %>%
    mutate(variable = "Attribute")
  
  plot_df <- bind_rows(rim_df, base_df, att_df)
  
  if (nrow(plot_df) == 0) {
    message("Skipping Type ", type_input, " Size ", size_input, " (no data)")
    return(NULL)
  }
  ...
  ggplot(plot_df, aes(x = category, y = count)) +
    ...
    theme_minimal() +
    labs(
      title = paste("Type", type_input, "| Size", size_input),
      x = NULL,
      y = "Count"
    ) +
    theme(
...
combos <- clean_data %>%
  distinct(type_category, size_category)

for (i in seq_len(nrow(combos))) {
  
  p <- plot_type_size(
    combos$type_category[i],
    combos$size_category[i]
  )
...
This section will understand which sites have which types, bases, sizes, rims, and attributes
...
# =========================================================
# STEP 1: PREP DATA (fix types + reshape)
# =========================================================

plot_data <- ceramics_data %>%
  mutate(
    type_category = as.character(type_category),
    size_category = as.character(size_category),
    rim_category  = as.character(rim_category),
    base_category = as.character(base_category),
    att_category  = as.character(att_category)   # 👈 NEW
  ) %>%
  pivot_longer(
    cols = c(type_category, size_category, rim_category, base_category, att_category),
    names_to = "variable",
    values_to = "category"
  ) %>%
  filter(!is.na(category), category != "", category != "special") %>%
  group_by(site_name, date, variable, category) %>%
  summarise(count = n(), .groups = "drop")
...
plot_site_date <- function(site_input, date_input) {
  
  df <- plot_data %>%
    filter(site_name == site_input, date == date_input)
  ...
  ggplot(df, aes(x = category, y = count)) +
    ...
    theme_minimal() +
    labs(
      title = paste("Site:", site_input, "| Date:", date_input),
      x = NULL,
      y = "Count"
    ) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# =========================================================
# STEP 3: LOOP THROUGH ALL SITE × DATE
# =========================================================
...
-------------------------------------------------------------
# Descritive statistics per type_category per size_category #
-------------------------------------------------------------
...
clean_data <- ceramics_data %>%
  mutate(
    type_category = as.factor(type_category),
...
long_data <- clean_data %>%
  pivot_longer(
    cols = c(actual_width, actual_diameter, actual_height, litres),
    names_to = "variable",
    values_to = "value"
  ) %>%
  filter(!is.na(value))
...
descriptive_table <- long_data %>%
  group_by(type_category, size_category, variable) %>%
  summarise(
    n       = n(),
    min     = min(value),
    max     = max(value),
  ...
  arrange(type_category, size_category, variable)
...
write.xlsx(
  descriptive_table,
  file = "descriptive_statistics_by_type_size.xlsx"
)

cat("\n✅ Excel file saved as: descriptive_statistics_by_type_size.xlsx\n")

# =========================================================
# STEP 5: PLOT FUNCTION (UNCHANGED)
# =========================================================

plot_type_stats <- function(type_input) {
  ...
  ggplot(df, aes(x = size_category, y = value)) +
    ...
    labs(
      title = paste("Descriptive Statistics for Vessel Type", type_input),
      x = "Size Category",
      y = "Value"
    )
}
...
types <- unique(long_data$type_category)
...
types <- unique(descriptive_table$type_category)

for (t in types) {
  
  df <- descriptive_table %>%
    filter(type_category == t)
  
  addWorksheet(wb, paste0("Type_", t))
  writeData(wb, sheet = paste0("Type_", t), df)
}
...
#cat("\n✅ Excel file saved with values rounded to 1 decimal.\n")
#getwd()
```
...
```{r, echo=TRUE}
ggplot(
  ceramics_data %>% filter(!is.na(litres) & litres != 0),
  aes(x = litres)
) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 10,
                 fill = "lightblue", color = "black") +
  ...
  scale_x_continuous(breaks = seq(0, 60, by = 10)) +
  labs(
    title = "Distribution and Density of Volume",
    x = "Volume (litres)",
    y = "Density"
  ) +
  theme_minimal(base_size = 13)
...
- Calculates the average silhouette width for k = 2–10.
...
for (type in type_categories) {
  ...
  if (nrow(numeric_data) < 3) {
    cat("  ⚠️ Skipped:", type, "– not enough data points (n =", nrow(numeric_data), ")\n\n")
    next
  }
...
---------------------------------------------------------------------
# 1 Cluster analysis per vessel type including orifice, width, height
---------------------------------------------------------------------
...
ggplot(optimal_clusters, aes(x = factor(type_category), y = optimal_k)) +
  ...
  geom_text(aes(label = paste0("k=", optimal_k, "\n(n=", n_datapoints, ")")),
            vjust = -0.6, size = 3) +
  labs(
    title = "Optimal Number of Clusters per Vessel Type (orifice, width, height)",
    x = "Vessel Type",
...
Gower distance (via the daisy() function in R’s cluster package) allows you to mix:
...
- It’s robust to outliers — very important when dealing with rare or atypical vessel types.

The code uses "pam(gower_dist, k)" which clusters the vessels based on their Gower dissimilarity matrix.

----------------------------------------------------------------------------------------------
# 1.1 Cluster analysis per vessel type including width, height, orifice, rim, base, attributes
----------------------------------------------------------------------------------------------
...
find_optimal_clusters_mixed <- function(df) {
  mixed_data <- df %>% select(actual_width, actual_diameter, actual_height, rim_category, att_category, base_category)
...
clustered_data <- ceramics_filtered %>%
  group_by(type_category) %>%
  group_modify(~{
    mixed_data <- .x %>% select(actual_width, actual_diameter, actual_height, rim_category, att_category, base_category)
    gower_dist <- daisy(mixed_data, metric = "gower")
...
cluster_summary <- clustered_data %>%
  ...
  summarise(
    ...
    att_types = paste(unique(att_category), collapse = ", "),
    .groups = "drop"
  )
...
ggplot(optimal_clusters, aes(x = factor(type_category), y = optimal_k)) +
  ...
  labs(
    title = "Optimal Number of Clusters per Vessel Type (orifice, width, height, rim, base, attribtues)",
    x = "Vessel Type",
    y = "Optimal Cluster Count"
  ) +
...
--------------------------------------------------------------------------------------------------------
# 1.2 Cluster analysis per vessel type including orifice, width, height, collapsed rim, base, attributes
--------------------------------------------------------------------------------------------------------
...
# Function to find optimal clusters (mixed numeric + categorical)
find_optimal_clusters_mixed <- function(df) {
  mixed_data <- df %>% select(actual_width, actual_diameter, actual_height, rim_collapsed, att_category, base_category)
  gower_dist <- daisy(mixed_data, metric = "gower")
  max_k <- min(10, nrow(df) - 1)
  if (nrow(df) < 3) return(NA)
...
clustered_data <- ceramics_filtered %>%
  group_by(type_category) %>%
  group_modify(~{
    mixed_data <- .x %>% select(actual_width, actual_diameter, actual_height, rim_collapsed, att_category, base_category)
    gower_dist <- daisy(mixed_data, metric = "gower")
...
cluster_summary <- clustered_data %>%
  ...
  summarise(
    vessel_numbers = paste(unique(vessel_number), collapse = ", "),
    rim_types = paste(unique(rim_collapsed), collapse = ", "),
    base_types = paste(unique(base_category), collapse = ", "),
    att_types = paste(unique(att_category), collapse = ", "),
    .groups = "drop"
  )
...
ggplot(optimal_clusters, aes(x = factor(type_category), y = optimal_k)) +
  ...
  geom_text(aes(label = paste0("k=", optimal_k, "\n(n=", n_datapoints, ")")),
            vjust = -0.6, fontface = "plain", size = 3) +
  labs(
    title = "Optimal Number of Clusters per Vessel Type (orifice, width, height, coll. rim, base, attribtues)",
    x = "Vessel Type",
    y = "Optimal Cluster Count"
  ) +
  expand_limits(y = max(optimal_clusters$optimal_k, na.rm = TRUE) + 2) +
...
----------------------------------------------------------------------------------------
# 1.3 Cluster analysis per vessel type including litres, collapsed rim, base, attributes
----------------------------------------------------------------------------------------

OBS: Types 14+15 are not included because there is not calculated any litres!
...
# Function to find optimal clusters (mixed numeric + categorical)
find_optimal_clusters_mixed <- function(df) {
  mixed_data <- df %>% select(litres, rim_collapsed, att_category, base_category)
  gower_dist <- daisy(mixed_data, metric = "gower")
  max_k <- min(10, nrow(df) - 1)
  if (nrow(df) < 3) return(NA)
...
clustered_data <- ceramics_filtered %>%
  group_by(type_category) %>%
  group_modify(~{
    mixed_data <- .x %>% select(litres, rim_collapsed, att_category, base_category)
    gower_dist <- daisy(mixed_data, metric = "gower")
...
ggplot(optimal_clusters, aes(x = factor(type_category), y = optimal_k)) +
  ...
  geom_text(aes(label = paste0("k=", optimal_k, "\n(n=", n_datapoints, ")")),
            vjust = -0.6, fontface = "plain", size = 3) +
  labs(
    title = "Optimal Number of Clusters per Vessel Type (litres, coll. rim, base, attributes)",
    x = "Vessel Type",
    y = "Optimal Cluster Count"
  ) +
  expand_limits(y = max(optimal_clusters$optimal_k, na.rm = TRUE) + 2) +
...
----------------------------------------------------------------------------------------
# 1.4 Cluster analysis per vessel type including size, collapsed rim, base, attributes
----------------------------------------------------------------------------------------
...
# Function to find optimal clusters using all-categorical data
find_optimal_clusters_mixed <- function(df) {
  if (nrow(df) < 3) return(NA)   # cannot cluster
  
  mixed_data <- df %>% select(size_category, rim_collapsed, att_category, base_category)
  gower_dist <- daisy(mixed_data, metric = "gower")
  
  max_k <- min(10, nrow(df) - 1)
  
  sil_values <- map_dbl(2:max_k, function(k) {
    pam_fit <- pam(gower_dist, k, diss = TRUE)
    pam_fit$silinfo$avg.width
  })
...
# Assign clusters for each vessel type
clustered_data <- ceramics_filtered %>%
  group_by(type_category) %>%
  group_modify(~{
    if (nrow(.x) < 3) {
    ...
    mixed_data <- .x %>% select(size_category, rim_collapsed, att_category, base_category)
    gower_dist <- daisy(mixed_data, metric = "gower")
    ...
    if (is.na(optimal_k) || optimal_k < 2) {
      ...
      pam_fit <- pam(gower_dist, optimal_k, diss = TRUE)
      .x$cluster <- pam_fit$clustering
    }
...
cluster_summary <- clustered_data %>%
  ...
  summarise(
    ...
    base_types = paste(unique(base_category), collapse = ", "),
    att_types = paste(unique(att_category), collapse = ", "),
    size_types = paste(unique(size_category), collapse = ", "),
    .groups = "drop"
  )
...
# Plot optimal k per vessel type
optimal_clusters <- ceramics_filtered %>%
  group_by(type_category) %>%
  summarise(
    n_datapoints = n(),
...
ggplot(optimal_clusters, aes(x = factor(type_category), y = optimal_k)) +
  ...
  geom_text(aes(label = paste0("k=", optimal_k, "\n(n=", n_datapoints, ")")),
            vjust = -0.6, size = 3) +
  labs(
    title = "Optimal Clusters per Vessel Type (all categorical variables)",
    x = "Vessel Type",
    y = "Optimal Cluster Count"
  ) +
  theme_minimal(base_size = 8) +
...
-------------------------------------------------------------------------------------------------------------------
# 1.5 Cluster analysis per vessel type including orifice, width, height, size cat., collapsed rim, base, attributes
-------------------------------------------------------------------------------------------------------------------

THIS IS THE ANALYSIS USED IN PHD AND IN THE APPENDIX

NOTES:

- When there is no data on the base of the vessel then they are treated as having a round base (Bro)
- When there is no attributes/plain the analysis show the most common attribute pr cluster. If I did not do it like this, all the vessels would be plain
...
# Function to find optimal clusters (mixed numeric + categorical)
find_optimal_clusters_mixed <- function(df) {
  mixed_data <- df %>% select(actual_width, actual_diameter, actual_height, size_category, rim_collapsed, att_category, base_category)
  gower_dist <- daisy(mixed_data, metric = "gower")
  max_k <- min(10, nrow(df) - 1)
  if (nrow(df) < 3) return(NA)
...
clustered_data <- ceramics_filtered %>%
  group_by(type_category) %>%
  group_modify(~{
    mixed_data <- .x %>% select(actual_width, actual_diameter, actual_height, size_category, rim_collapsed, att_category, base_category)
    gower_dist <- daisy(mixed_data, metric = "gower")
...
ggplot(optimal_clusters, aes(x = factor(type_category), y = optimal_k)) +
  ...
  geom_text(aes(label = paste0("k=", optimal_k, "\n(n=", n_datapoints, ")")),
            vjust = -0.6, fontface = "plain", size = 3) +
  labs(
    title = "Optimal Number of Clusters per Vessel Type (orifice, width, height, size cat., coll. rim, base, attribtues)",
    x = "Vessel Type",
    y = "Optimal Cluster Count"
  ) +
  expand_limits(y = max(optimal_clusters$optimal_k, na.rm = TRUE) + 2) +
...
10–30	Possible, but interpret very cautiously
30–50	Can see preliminary structure, better if few clusters
...
----------------------------------------------
# EXPORT DESCRIPTIVE STATISTICS FOR METHOD 1.5
----------------------------------------------
...
# ---- 1. Numeric descriptive statistics ----
numeric_stats <- clustered_data %>%
...
# ---- 2. Most common categorical values ----
most_common <- function(x) {
  tab <- sort(table(x), decreasing = TRUE)
  paste0(names(tab)[1], " (n=", tab[1], ")")
}

categorical_summary <- clustered_data %>%
  group_by(type_category, cluster) %>%
  summarise(
    most_common_rim  = most_common(rim_collapsed),
    most_common_base = most_common(base_category),
    most_common_size = most_common(size_category),
    most_common_att  = most_common(att_category),
    .groups = "drop"
  )

# ---- 3. Full frequency tables (long format) ----
frequency_tables <- clustered_data %>%
  pivot_longer(
    cols = c(rim_collapsed, base_category, size_category, att_category),
    names_to = "variable",
    values_to = "category"
  ) %>%
  group_by(type_category, cluster, variable, category) %>%
  summarise(count = n(), .groups = "drop") %>%
  arrange(type_category, cluster, variable, desc(count))

# ---- 4. Vessel-level reference table (NEW) ----
vessel_reference <- clustered_data %>%
  select(
    site_name,
    vessel_number,
    type_category,
  ...
  arrange(type_category, cluster, site_name)

# ---- 5. Cluster distribution by site ----
cluster_by_site <- clustered_data %>%
  group_by(type_category, cluster, site_name) %>%
  summarise(n = n(), .groups = "drop") %>%
  arrange(type_category, cluster, desc(n))

# ---- 6. Cluster distribution by phase (date) ----
cluster_by_phase <- clustered_data %>%
  group_by(type_category, cluster, date) %>%
  summarise(n = n(), .groups = "drop") %>%
  arrange(type_category, cluster, date)

# 7. Dominant cluster per site × phase

dominant_cluster <- clustered_data %>%
  group_by(site_name, date, cluster) %>%
  summarise(n = n(), .groups = "drop") %>%
  group_by(site_name, date) %>%
...
# 8. Cluster trajectory over time (per type)

cluster_trajectory <- clustered_data %>%
  group_by(type_category, date, cluster) %>%
  summarise(n = n(), .groups = "drop") %>%
  arrange(type_category, date, cluster)
...
write.xlsx(
  list(
    ...
    "Frequencies"           = frequency_tables,
    "Vessel_Reference"      = vessel_reference,
    "Cluster_by_Site"       = cluster_by_site,
    "Cluster_by_Phase"      = cluster_by_phase,
    "Dominant_Cluster"      = dominant_cluster,     # NEW
    "Cluster_Trajectory"    = cluster_trajectory    # NEW
  ),
  file = "cluster_descriptive_statistics.xlsx"
)

cat("\n✅ Excel file exported with full analytical framework\n")

```


-------------------------------------------------------------
# The following cluster analysis is based on method 1.5 above
-------------------------------------------------------------
...
gower_dist <- daisy(
  ceramics_subset[, c("actual_width", "actual_diameter", "actual_height", "size_category", "rim_collapsed", "att_category", "base_category")],
  metric = "gower"
)
...
cluster_summary <- ceramics_subset %>%
  select(vessel_number, site_name, size_category, rim_collapsed, att_category, base_category, cluster) %>%
  arrange(cluster, site_name)

cat("\n=== Cluster Summary Table ===\n")
print(cluster_summary, n = nrow(cluster_summary))

# --- Step 6: Cluster dependency profiles (means, medians, min–max, etc.) ---
...
#    sd_litres = sd(litres, na.rm = TRUE),
    ...
    most_common_att = names(sort(table(att_category), decreasing = TRUE))[1],
    most_common_base = names(sort(table(base_category), decreasing = TRUE))[1],
    most_common_size = names(sort(table(size_category), decreasing = TRUE))[1]
  )
