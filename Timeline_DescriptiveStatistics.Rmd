...
- What attribute category is dominant overall?
...
cowplot::plot_grid(bar_plot_vessel, legend_plot_vessel, ncol = 2, rel_widths = c(3, 1.5))  # Reduced legend width
...
# Arrange the bar plot and the legend side by side with adjusted widths
cowplot::plot_grid(bar_plot_rim, legend_plot_rim, ncol = 2, rel_widths = c(3, 1.5))  # Reduced legend width
...
# Arrange the bar plot and the legend side by side with adjusted widths
cowplot::plot_grid(bar_plot_base, legend_plot_base, ncol = 2, rel_widths = c(3, 1.5))  # Reduced legend width
...
legend_table_size <- data.frame(
  ...
  Description = c("<14 cm", "15-24.9 cm", "25-34.9 cm", "35-44.9 cm", ">45 cm")
)
...
# Arrange the bar plot and the legend side by side with adjusted widths
cowplot::plot_grid(bar_plot_size, legend_plot_size, ncol = 2, rel_widths = c(3, 1.5))  # Reduced legend width
...
# Create a data frame to count the number of data points for each attribute
attribute_counts <- data.frame(table(ceramics_subset$attributes))
colnames(attribute_counts) <- c("attributes", "Count")

# Ensure 'attributes' is treated as a factor for correct order in the plot
attribute_counts$attributes <- factor(attribute_counts$attributes, 
                                      levels = legend_table_attribute$Abbreviation)

# Create the bar plot of 'Data points per attribute category'
bar_plot_attribute <- ggplot(attribute_counts, aes(x = attributes, y = Count)) +
  geom_col(fill = "lightblue") +
...
# Legend table for attribute descriptions
...
# Convert the legend table to a plot
legend_plot_attribute <- ggplot(legend_table_attribute, 
                                aes(x = "", y = Attribute_Type, 
                                    label = paste(Abbreviation, "=", Description))) +
  geom_text(hjust = 0, size = 3) +
  theme_void() +
  theme(plot.margin = margin(5, 20, 5, -140))

# Combine bar plot and legend side by side
