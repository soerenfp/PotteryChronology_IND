...
---------------------------------------------
# STEP 6: Plot of vessel type counts per site
---------------------------------------------
...
# Reorder type_category to ensure numeric order in the legend (OBS: I think I need to change this area so that my x-axis is in numeric order)
data_count_VS01 <- data_count_VS01 %>%
  mutate(type_description = factor(type_description, levels = type_descriptions_VS01$type_description))
...
Damn this looks good! I need to change the x-axis so that also is in numeric order, but that is for tomorrow

# Interpretation of 02_timelinedata results

- I need to look closer at the results later
...
# Reshape the data for plotting (long format for ggplot)
data_long_VS02 <- data_wide_VS02 %>%
  pivot_longer(cols = -site_name, names_to = "type_category", values_to = "count")

# Reorder type_category to ensure numeric order in the legend
data_long_VS02$type_category <- factor(data_long_VS02$type_category, levels = as.character(1:15)) # OBS: 15 colors
...
plot1_VS02 <- ggplot(data_first_third_VS02, aes(x = site_name, y = count, fill = type_category)) +
  geom_bar(stat = "identity", position = "dodge") +  # Position bars side by side
  scale_fill_viridis(discrete = TRUE, option = "D") +  # Viridis palette with 14+ colors
  theme_minimal() +
  labs(
    x = "Site Name",
    y = "Count",
    title = "Count of Vessel Type Categories pr. Site (1/3)",
    fill = "Type Category"  # Title for the legend
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Plot 2: Second third of the sites
plot2_VS02 <- ggplot(data_second_third_VS02, aes(x = site_name, y = count, fill = type_category)) +
  geom_bar(stat = "identity", position = "dodge") +  # Position bars side by side
  scale_fill_viridis(discrete = TRUE, option = "D") +  # Viridis palette with 14+ colors
  theme_minimal() +
  labs(
...
plot3_VS02 <- ggplot(data_third_third_VS02, aes(x = site_name, y = count, fill = type_category)) +
  geom_bar(stat = "identity", position = "dodge") +  # Position bars side by side
  scale_fill_viridis(discrete = TRUE, option = "D") +  # Viridis palette with 14+ colors
  theme_minimal() +
  labs(
