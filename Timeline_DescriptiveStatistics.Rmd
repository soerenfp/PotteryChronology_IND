...
# Assuming your dataset is named 'ceramics_subset' with columns 'site_name' and 'rim_category'
# Count the occurrences of rim_category per site
data_count_RS02 <- ceramics_subset %>%
  group_by(site_name, rim_category) %>%
  summarise(count = n(), .groups = 'drop')

# Add a mapping of rim_category to type descriptions (example mapping)
type_descriptions_RS02 <- data.frame(
  rim_category = c("Sr", "Sp", "Sf", "Sb", "Es", "Ec", "Ecc", "Ef", "Efl", "Ep", 
                   "Et", "Esg", "Emg", "Vs", "Vc", "Oth", "Otc", "Otf", "Ith", 
                   "Itc", "Itf", "Th", "Tc", "Tf", "Ca", "I", "Y"),
  rim_description = c("Simple, rounded", "Simple, pointy", "Simple, flattened", 
...
# Ensure the rim_category columns are both character columns
#data_count_RS02$rim_category <- as.character(data_count_RS02$rim_category)
#type_descriptions_RS02$rim_category <- as.character(type_descriptions_RS02$rim_category)

# Merge descriptions into the data
data_count_RS02 <- data_count_RS02 %>%
  left_join(type_descriptions_RS02, by = "rim_category")

# Ensure the x-axis (rim_category) and legend are in numeric order
#data_count_RS02 <- data_count_RS02 %>%
#  mutate(rim_category = factor(rim_category, levels = as.character(1:27)),
#         rim_description = factor(rim_description, levels = type_descriptions_RS02$rim_description))

# Pivot the data to wide format: Each rim_category becomes a separate column
data_wide_RS02 <- data_count_RS02 %>%
  select(-rim_description) %>% # Exclude type_description for the wide format
...
data_long_RS02 <- data_wide_RS02 %>%
  ...
  mutate(rim_category = factor(rim_category, levels = as.character(1:27))) # Restore factor order
