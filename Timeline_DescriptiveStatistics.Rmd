...
ggplot(ceramics_data_clean, aes(x = actual_diameter, fill = factor(type_category))) +
  ...
  facet_wrap(~ type_category, scales = "free_y") +
  labs(
    title = "Density Plot of Actual Diameter by Vessel Type",
    x = "Actual Diameter (cm)",
    y = "Density"
  ) +
  theme_minimal(base_size = 13) +
...
ggplot(ceramics_long, aes(x = Value, fill = Measurement)) +
  ...
  facet_wrap(~ type_category, scales = "free_y") +
  scale_fill_manual(values = c("Width" = "#1f77b4", "Height" = "#2ca02c", "Diameter" = "#ff7f0e")) +
  labs(
...
NICK: before doing cluster analysis, define from the groups above which vessel types are worth clustering together (the ones that have more than one peak). So, do a cluster analysis per type group. 

DO A CLUSTER ANALYSIS PER VESSEL TYPE GROUP!!!
...
scaled_data <- scale(ceramics_subset[, c("actual_width", "actual_diameter", "actual_height")])

# --- Step 3: Perform clustering (k = 2, since that was optimal before) ---
set.seed(123)
kmeans_result <- kmeans(scaled_data, centers = 2, nstart = 25)

ceramics_subset$cluster <- as.factor(kmeans_result$cluster)
