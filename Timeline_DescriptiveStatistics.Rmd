...
So, now I created a data set "ceramics_subset" of the columns noted above and removed rows from the "actual_diameter" column that contained 0.0 values. The data set contains 576 vessels (611 original).

I need the count of the "actual_diameter" 0.0 also, so I turned off the filter()
...
```
OBS: I think I need to include the vessels with 0.0 values in actual_diameter for the total count. But otherwise the plot looks nice!
...
# Create a data frame to count the number of data points for each vessel type
base_counts <- data.frame(table(ceramics_data$base_category))
...
legend_table <- data.frame(
  Base_Type = 1:7,
  Abbreviation = c("Bf", "Br", "Bpe", "Bl", "Bc", "Bpo", "Sp"),
