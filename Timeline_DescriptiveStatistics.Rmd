...
OBS: This is the one used for the appendix
...
### Clean code base on method 1.5 above (TRIAL) ###
...
# --- Step 1: Filter for one vessel type ---
vessel_type_selected <- 1

ceramics_subset <- ceramics_data %>%
  filter(type_category == vessel_type_selected) %>%
  mutate(
    base_category = ifelse(is.na(base_category) | base_category == "", "Bro", base_category),
    actual_width = na_if(actual_width, 0),
    actual_diameter = na_if(actual_diameter, 0),
    actual_height = na_if(actual_height, 0),
    litres = na_if(litres, 0),
    litres = round(litres, 2),
    rim_collapsed = fct_drop(as.factor(rim_collapsed)),
    att_category = fct_drop(as.factor(att_category)),
    base_category = fct_drop(as.factor(base_category)),
    size_category = fct_drop(as.factor(size_category))
  ) %>%
  filter(
    !is.na(actual_width) |
    !is.na(actual_diameter) |
...
cat("\nRows used for clustering:", nrow(ceramics_subset), "\n")
...
gower_dist <- daisy(ceramics_subset[, gower_vars], metric = "gower")

# --- Step 3: Optimal k ---
max_k <- min(10, nrow(ceramics_subset) - 1)
sil_widths <- rep(NA, max_k)
...
numeric_stats <- ceramics_subset %>%
  group_by(cluster) %>%
  summarise(
    n = n(),

    width_mean   = mean(actual_width, na.rm = TRUE),
    width_median = median(actual_width, na.rm = TRUE),
    width_min    = min(actual_width, na.rm = TRUE),
    width_max    = max(actual_width, na.rm = TRUE),

    diameter_mean   = mean(actual_diameter, na.rm = TRUE),
    diameter_median = median(actual_diameter, na.rm = TRUE),

    height_mean   = mean(actual_height, na.rm = TRUE),
    height_median = median(actual_height, na.rm = TRUE),

    litres_mean = mean(litres, na.rm = TRUE),
    litres_median = median(litres, na.rm = TRUE),

    .groups = "drop"
  )

cat("\n==============================\n")
cat("DESCRIPTIVE STATISTICS (SIZE)\n")
cat("==============================\n")
...
categorical_profiles <- ceramics_subset %>%
  group_by(cluster) %>%
  summarise(
...
cat("\n========================================\n")
cat("MOST COMMON CATEGORICAL CHARACTERISTICS\n")
cat("========================================\n")
print(as.data.frame(categorical_profiles), row.names = FALSE)

# =========================================================
# === OPTIONAL: FULL FREQUENCY TABLES =====================
# =========================================================

cat("\n--- Rim frequencies by cluster ---\n")
print(
  ceramics_subset %>%
    count(cluster, rim_collapsed) %>%
    arrange(cluster, desc(n)),
  n = Inf
)

cat("\n--- Base frequencies by cluster ---\n")
print(
  ceramics_subset %>%
    count(cluster, base_category) %>%
    arrange(cluster, desc(n)),
  n = Inf
)

cat("\n--- Size category frequencies by cluster ---\n")
print(
  ceramics_subset %>%
    count(cluster, size_category) %>%
    arrange(cluster, desc(n)),
  n = Inf
)

cat("\n--- Attribute frequencies by cluster ---\n")
print(
  ceramics_subset %>%
    count(cluster, att_category) %>%
    arrange(cluster, desc(n)),
  n = Inf
)
